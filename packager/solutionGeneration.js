...
component_filter_map[appModulesPath] = (dir)=>{
  ...
  fse.copySync(dir.split('/').slice(0, 3).join('/'), `${changesPath}/${appModulesPath}/${dir.split('/')[2]}`);
};
...
component_filter_map[entitiesPath] = (dir)=>{
  ...
  fse.copySync(dir.split('/').slice(0, 3).join('/'), `${changesPath}/${entitiesPath}/${dir.split('/')[2]}`);
};
component_filter_map[interactionCentricDashboardsPath] = (dir)=>{
  console.log(`processing interactionCentricDashboard path ${dir}`);
  fse.copySync(dir, `${changesPath}/${interactionCentricDashboardsPath}/${dir.split('/')[2]}`);
};
...
component_filter_map[otherPath] = (dir)=>{
  ...
  if(!dir.match(/solution\.xml/i)
    && !dir.match(/customizations\.xml/i)){
...
component_filter_map[pluginAssembliesPath] = (dir)=>{
  ...
  if(!dir.match(/\.data\.xml/i)){
    ...
    fse.copySync(dir, `${changesPath}/${pluginAssembliesPath}/${assemblyPath}/${fileName + ".dll"}`);
    fse.copySync(`${basePath}/${assemblyPath}/${fileName + '.dll.data.xml'}`, `${changesPath}/${pluginAssembliesPath}/${assemblyPath}/${fileName + ".dll.data.xml"}`);
  }
};
component_filter_map[sdkMessageProcessingStepsPath] = (dir)=>{
  console.log(`processing sdkMessageProcessingStep path ${dir}`);
  fse.copySync(dir, `${changesPath}/${sdkMessageProcessingStepsPath}/${dir.split('/')[2]}`);
};
component_filter_map[webResourcesPath] = (dir)=>{
  ...
  if(!dir.match(/\.data\.xml/i)){
    ...
    fse.copySync(dir + '.data.xml', `${changesPath}/${webResourcesPath}/${compPath + ".data.xml"}`);
  }
};
component_filter_map[workflowsPath] = (dir)=>{
  ...
  if(!dir.match(/\.data\.xml/i)){
    ...
    fse.copySync(dir + '.data.xml', `${changesPath}/${workflowsPath}/${fileName + ".data.xml"}`);
  }
};
